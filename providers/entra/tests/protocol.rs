...
async fn process(input: &[u8]) -> std::process::Output {
    let mut child = tokio::process::Command::new(env!("CARGO_BIN_EXE_permesh-provider-entra"))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .kill_on_drop(true)
...
async fn binary_describes_valid_setup_without_credentials_or_network() {
    let input = format!(
        "{}\n{}\n",
        json!({"protocol":3,"id":"handshake","method":"handshake","instance":"entra"}),
    ...
    let mut decoder = SetupDecoder::new(
        "entra",
        "entra",
        Some(&permesh_provider_entra::provider_metadata().capabilities),
    )
    .unwrap();
    ...
    assert_eq!(
        questions
            ...
            .collect::<Vec<_>>(),
        ["tenant_id", "include_service_principals", "token"]
    );
}
#[tokio::test]
async fn binary_rejects_credentials_in_configuration_and_unknown_fields_without_reflection() {
    let input = format!(
        ...
        json!({"protocol_version":1,"id":"discover","method":"discover","configuration":{"tenant_id":"11111111-1111-1111-1111-111111111111","token":"SENTINEL"},"credentials":{"token":"SENTINEL"}})
    );
    let result = process(input.as_bytes()).await;
    assert!(!result.status.success());
...
async fn binary_honors_queued_cancellation_before_discovery() {
    let input = format!(
        "{}\n{}\n{}\n",
        json!({"protocol_version":1,"id":"handshake","method":"handshake","instance":"entra","operation":"discover"}),
        json!({"protocol_version":1,"id":"discover","method":"discover","configuration":{"tenant_id":"11111111-1111-1111-1111-111111111111"},"credentials":{"token":"SENTINEL"}}),
...
async fn binary_rejects_draft_discovery_and_unselected_operations_before_credentials() {
    for handshake in [
        json!({"protocol":2,"id":"handshake","method":"handshake","instance":"entra"}),
        json!({"protocol_version":1,"id":"handshake","method":"handshake","instance":"entra"}),
        json!({"protocol_version":1,"id":"handshake","method":"handshake","instance":"entra","operation":"describe"}),
    ] {
...
async fn negotiated_network_routes_provider_api_through_explicit_proxy() {
    process_network::check_proxy(
        env!("CARGO_BIN_EXE_permesh-provider-entra"),
        "entra",
        serde_json::json!({"tenant_id":"11111111-1111-1111-1111-111111111111"}),
