...
async fn native_setup_and_cancel_are_offline_and_never_reflect_input() {
    for describe in [true, false] {
        let mut child = tokio::process::Command::new(env!("CARGO_BIN_EXE_permesh-provider-gitlab"))
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .kill_on_drop(true)
        ...
        if describe {
            let mut decoder = permesh_provider_protocol::SetupDecoder::new(
                "gitlab",
                "test-main",
                Some(&permesh_provider_gitlab::provider_metadata().capabilities),
            )
            .unwrap();
